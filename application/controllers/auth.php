...
class Auth extends MY_Controller {
	...
	function index()
	{
		...
		else
		{
			...
			$this->data['content'] = $this->load->view('/auth/index', $this->data, TRUE);
            $this->render('default', $this->data);
		}
	}
	...
	function login()
	{
		...
		{ //check to see if the user is logging in
			...
			{ //if the login was un-successful
				...
				redirect($this->config->item('base_url').'auth/login'); //use redirects instead of loading views for compatibility with MY_Controller libraries
			}
		}
		else
		{  //the user is not logging in so display the login page
            ...
            $this->render('admin-public', $this->data);
		}
	}
	...
	function logout()
	{
        ...
        $this->session->set_flashdata('alert', 'Logged out successfully');
	...
	function change_password()
	{
		...
		{ //display the form
			...
			//render
			$this->data['content'] = $this->load->view('auth/change_password', $this->data, TRUE);
            $this->render('default', $this->data);
		}
		else
		{
			...
			else
			{
				...
				redirect($this->config->item('base_url').'auth/change_password');
			}
		}
	}
	...
	function forgot_password()
	{
		...
		else
		{
			...
			{ //if there were no errors
				$this->session->set_flashdata('message', $this->ion_auth->messages());
				redirect($this->config->item('base_url').'auth/login'); //we should display a confirmation page here instead of the login page
			}
			else
			{
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect($this->config->item('base_url').'auth/forgot_password');
			}
		}
	}
	...
	function deactivate($id = NULL)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->data['content'] = $this->load->view('auth/deactivate_user', $this->data, TRUE);
	...
	function create_user()
	{
		...
		{ //display the create user form
			...
			$this->data['content'] = $this->load->view('auth/create_user', $this->data, TRUE);
            $this->render('default', $this->data);
		}
	}
